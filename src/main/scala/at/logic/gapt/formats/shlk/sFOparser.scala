  ...
  def parseSequent( txt: String ): FSequent = {
    class SequentParser extends JavaTokenParsers with at.logic.gapt.language.lambda.types.Parsers {
      ...
      def s_term: Parser[SchemaExpression] = "[g,h][0-9]*".r ~ "(" ~ intTerm ~ "," ~ term ~ ")" ^^ {
        case name ~ "(" ~ i ~ "," ~ args ~ ")" => { sTerm( name, i.asInstanceOf[IntegerTerm], args :: Nil ) }
      }
      ...
      def var_atom: Parser[SchemaFormula] = regex( new Regex( "[p-r]" ) ) ~ "(" ~ repsep( term, "," ) ~ ")" ^^ {
        case x ~ "(" ~ params ~ ")" => {
      ...
      def var_func: Parser[SchemaExpression] = regex( new Regex( "[p-r]" ) ) ~ "(" ~ repsep( term, "," ) ~ ")" ^^ {
        case x ~ "(" ~ params ~ ")" =>
          SchemaFunction( SchemaVar( x, FunctionType( Tindex -> Tindex, params map ( _.exptype ) ) ), params )
      }
      def const_func: Parser[SchemaExpression] = regex( new Regex( "[" + symbols + "a-tA-Z0-9]*" ) ) ~ "(" ~ repsep( term, "," ) ~ ")" ^^ {
        case x ~ "(" ~ params ~ ")" =>
