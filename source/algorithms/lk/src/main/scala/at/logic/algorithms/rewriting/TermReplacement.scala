...
import at.logic.language.fol.{FOLExpression, FOLFormula, FOLTerm, FOLVar, Substitution}
    ...
    if (pmap contains p) add_pmap(pmap,p) else
      p match {
        ...
        case Variant(clause, parent1, sub) =>
          ...
          val nsmap : Map[FOLVar, FOLExpression] = sub.map.asInstanceOf[Map[FOLVar,FOLExpression]] map ((x:(FOLVar, FOLExpression)) => (x._1, apply(x._2, smap)) )
        ...
        case Factor(clause, parent1, aux, sub) =>
          val (rpmap, rmap, rparent1) = if (pmap contains parent1) add_pmap(pmap, parent1) else rename_resproof(parent1, irules, smap, pmap)
          val nsub = Substitution(sub.map.asInstanceOf[Map[FOLVar,FOLExpression]] map ((x:(FOLVar, FOLExpression)) => (x._1, apply(x._2, smap)) ))
        ...
        case Resolution(clause, parent1, parent2, lit1, lit2, sub) =>
          ...
          //debug(2,"sub: "+sub)
          val nsub = Substitution(sub.map.asInstanceOf[Map[FOLVar,FOLExpression]] map ((x:(FOLVar, FOLExpression)) => {
            //debug(2,smap)
