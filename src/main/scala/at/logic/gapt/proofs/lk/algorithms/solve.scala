...
class SchemaProofStrategy extends ProofStrategy with at.logic.utils.logging.Logger {
  ...
  def findWeakQuantifier(seq: FSequent): Option[ProofStrategy.Action] = {
    import at.logic.language.schema.{dbTRS, SchemaConst}
...
private object SolveUtils extends Logger {
  ...
  def findSchematicAxiom(seq: FSequent) : Option[LKProof] = {
    val pair = for ( f <- seq.antecedent.toList;
      ...
      if ((f,g) match {
      case (Atom(f1, args1), Atom(g1, args2)) if (f1 == g1) => true
      case _ => false
      })
    ) yield {(f,g)}
    pair match {
      ...
      case (f,g)::_ => try {
        ...
        /*  (f,g) match {
          case (Atom(f1, args1), Atom(f2, args2)) if args1.head.arity < 2 =>

          case (Atom(f1, args1), Atom(f2, args2)) if args2.head.arity < 2 =>
