...
//    ("p",parseProof(input, "root"))::Nil
  ...
  def parseSequent(txt: String): FSequent = {
    class SequentParser extends JavaTokenParsers with Parsers {
     // def name = """[\\]*[a-z_0-9]*""".r
      def formula: Parser[SchemaFormula] = (atom | neg | and | or | indPred | imp | forall | exists) ^? {case trm: SchemaFormula => trm }
      def intTerm: Parser[IntegerTerm] = (sum | intConst | intVar | succ  )
      ...
      def indPred : Parser[SchemaFormula] = """[A-Z]*[0-9]*""".r ~ "(" ~ intTerm ~ "," ~ s_term ~ ")" ^^ {
        case x ~ "(" ~ l ~ "," ~ t ~ ")" => IndexedPredicate(x, l)
      }


      def term: Parser[SchemaExpression] = (fo_term | s_term | indexedVar | variable | constant | var_func | const_func)
      def s_term: Parser[SchemaExpression] = "[g,h][0-9]*".r ~ "(" ~ intTerm ~ "," ~ term ~ ")" ^^ {
        case name ~ "(" ~ i ~ "," ~ arg ~ ")" => sTerm(name, i.asInstanceOf[IntegerTerm], arg::Nil)
      }
      def fo_term: Parser[SchemaExpression] = "[f][0-9]*".r ~ "(" ~ term ~ ")" ^^ {
        case name ~ "(" ~ arg ~ ")" => {
          val v = SchemaFactory.createConst(StringSymbol(name), Ti -> Ti)
          SchemaApp(v, arg)
        }
      }
      def indexedVar: Parser[SchemaVar] = regex(new Regex("[u,v]")) ~ "(" ~ intTerm ~ ")" ^^ {
        case x ~ "(" ~ index ~ ")" => SchemaFactory.createVar(StringSymbol(x), Tindex -> Ti)
      }
      def variable: Parser[SchemaVar] = regex(new Regex("[x-z][0-9]*")) ^^ {case x => foVar(x)}
      def constant: Parser[SchemaConst] = regex(new Regex("[a-c][0-9]*")) ^^ {case x => SchemaFactory.createConst(StringSymbol(x), Ti)}
      ...
      def atom: Parser[SchemaFormula] = (var_atom | const_atom)
      ...
      def var_atom: Parser[SchemaFormula] = regex(new Regex("[p-r]")) ~ "(" ~ repsep(term,",") ~ ")" ^^ {case x ~ "(" ~ params ~ ")" => {
        Atom(SchemaVar(x, FunctionType(To, params map (_.exptype))), params)
      }}

      def const_atom: Parser[SchemaFormula] = regex(new Regex("[P-R][0-9]*")) ~ "(" ~ repsep(term,",") ~ ")" ^^ {
        case x ~ "(" ~ params ~ ")" =>
          Atom(SchemaConst(x, FunctionType(To, params map (_.exptype))), params)
      }
      def var_func: Parser[SchemaExpression] = regex(new Regex("[p-r]")) ~ "(" ~ repsep(term,",") ~ ")" ^^ {case x ~ "(" ~ params ~ ")"  =>
        Function(SchemaVar(x, FunctionType(Tindex->Tindex, params map (_.exptype))), params)
      }
      def const_func: Parser[SchemaExpression] = regex(new Regex("["+symbols+"a-tA-Z0-9]*")) ~ "(" ~ repsep(term,",") ~ ")"  ^^ {case x ~ "(" ~ params ~ ")"  =>
